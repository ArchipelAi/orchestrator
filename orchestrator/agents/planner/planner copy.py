...
async def planner(state: State) -> State:
    ...
    task_step, num_steps_proposed, new_step, response_outputs_backup, new_response_outputs = orep.update_request_parameters(
    ...
    return {
        **state,
        ...
        "projected_time_to_finish": state['projected_time_to_finish'] + len(best_response_body),

...
def update_request_parameters(best_response_body, num_step, response_outputs):
    ...
    return task_step, num_steps_proposed, num_step, response_outputs_backup, response_outputs
...
class SequencePlanner:
    ...
    def __init__(self, llm, tools, n_models, system_task):
        ...
        self.prompt = PromptTemplate(template=self.prompt_template, input_variables=self.input_variables)
        self.prompt.format(n_models=n_models, system_task=system_task, agent_scratchpad="")
...
class SolutionAgent:
    ...
    def __init__(self, llm, tools, n_models, task_step):
        self.llm = llm
        self.tools = tools
        self.n_models = n_models
        self.task_step = task_step
        self.prompt = PromptTemplate(template=self.prompt_template, input_variables=self.input_variables)
        self.prompt.format(n_models=n_models, task_step=task_step, agent_scratchpad="")
        self.agent_runnable = create_openai_functions_agent(llm, tools, self.prompt)

    def plan(self, n_models, task_step, agent_scratchpad='The current global task is {task_step}'):
        inputs = {
            "chat_history": [],  # Initial chat history
            "intermediate_steps": [],  # Initialize with an empty list
            "agent_scratchpad": agent_scratchpad,  # Initial value for agent_scratchpad
...
def generate_feature_vector(response_outputs, problem_type, problem_scope, num_step, n_models, best_response_backup, projected_time_to_finish):
    ...
    # Evaluate error rate
    error_rate = orep.evaluate_error_rate(best_response_body, best_response_backup, responses=None, num_step=num_step)
    if num_step == 0:
        projected_time_to_finish += len(best_response_body)
    else:
        pass
    # Evaluate environment complexity
    problem_scope_label, problem_type_label, ratio_steps_left = orep.evaluate_environment_complexity(problem_scope, problem_type, num_step, n_models, best_response_body, projected_time_to_finish)

    # Generate feature vector
    feature_vector = (max(act_max_scores), ag_rationality, model_type_label, agent_type_label, problem_scope_label, problem_type_label, num_step, n_models, projected_time_to_finish, ratio_steps_left, error_rate)
...
for i in range(n_models):
    ...
    sequence_planner = SequencePlanner(llm, tools, n_models, system_task) #can be modified to change llm type with alternating i
...
feature_vector, best_response_body = generate_feature_vector(response_outputs, problem_type, problem_scope, num_step, n_models, best_response_backup, projected_time_to_finish)

training_data.append(feature_vector)

#UPDATE REQUEST PARAMETERS
task_step, num_steps_proposed, num_step, response_outputs_backup, response_outputs = update_request_parameters(best_response_body, num_step, response_outputs)
...
for i in range(n_models):
    ...
    solution_agent = SolutionAgent(llm, tools, n_models, task_step)  # can be modified to change llm type with alternating i
